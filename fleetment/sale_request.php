...
<li><a class="dropdown-item" href="sale_request.php"><i class="bi bi-person-lines-fill me-2"></i>จัดการแผนเดินรถ (ฝ่ายขาย)</a></li>
  ...
  <h4 class="text-center fw-bold py-3 mb-4 text-white" style="background-color: #16325cff; border-radius: 0.5rem;">
    ขอเปลี่ยนแปลงคิว (ฝ่ายขาย)
  </h4>
  <div class="row g-4">
<!-- ซ้าย: วันที่ + เลือกสาย -->
<div class="col-md-4">
  <!-- ฟอร์มเลือกวันที่ + เลือกสายเดินรถ รวมอยู่ใน card เดียว -->
  <div class="card mb-4">
    <div class="card-header fw-bold">กำหนดวันที่ และ สายเดินรถ</div>
    <div class="card-body">
      <label for="common-date" class="form-label">วันที่สำหรับสร้างรายการ :</label>
      <input type="date" id="common-date" class="form-control" value="<?= htmlspecialchars($plan_date ?? '') ?>">
      ...
      <label for="route-select" class="form-label">สายเดินรถ :</label>
      <div class="d-flex gap-2 mb-2 ">
        <button type="button" class="btn btn-sm btn-outline-primary" id="select-all-routes-btn">เลือกทุกสาย</button>
        <button type="button" class="btn btn-sm btn-outline-secondary" id="clear-all-routes-btn">ล้างการเลือก</button>
      </div>
    </div>
      <select class="form-control" id="route-select" name="routes[]" multiple style="height: 300px;">
  <?php foreach ($all_routes as $route): ?>
    <option value="<?= htmlspecialchars($route['br_id']) ?>" 
            <?= in_array($route['br_id'], $route_selected ?? []) ? 'selected' : '' ?>>
  ...
  <?php endforeach; ?>
    ...
    <!-- ขวา: ตาราง request/reserve -->
    <div class="col-md-8">
      <div id="request-tables" style="min-height: 400px;">
        <!-- ตาราง request/reserve จะถูกโหลดและแสดงที่นี่ -->
      </div>
    </div>
  </div>
</div>
    ...
    Object.entries(request).forEach(([br_id, obj]) => {
        ...
        html += `<div class="card mb-4 shadow-sm">
            <div class="card-header bg-secondary text-white d-flex justify-content-between align-items-center">
                <h6 class="mb-0">สายเดินรถ : ${br_id}</h6>
            </div>
            <div class="card-body">
                ...
                <table class="table table-bordered align-middle table-sm">
                    <thead class="table-secondary">
                        <tr>
                            <th class="text-center fw-light">ลำดับ</th>
                            <th class="text-center fw-light">รหัสประจำคิว</th>
                            <th class="text-center fw-light">Queue request</th>
                            <th class="text-center fw-light">เวลา</th>
                            <th class="text-center fw-light">เวลาเดินทาง (นาที)</th>
                            <th class="text-center fw-light">Action</th>
                        </tr>
                    </thead>
            ...
            <tr>
                <td colspan="5">
                    ...
                    <div class="row g-2 align-items-center mt-2">
                        <div class="col-auto">จุดจอดขึ้น (ex driver คนที่ 2):</div>
                        <div class="col">${createExPointSelect(br_id, idx, exObj.start2, 'start', 2)}</div>
                        <div class="col-auto">จุดจอดลง (ex driver คนที่ 2):</div>
                ...
                <table class="table table-bordered align-middle table-sm">
                    <thead class="table-light">
                        <tr>
                            <th class="text-center fw-light">ลำดับ</th>
                            <th class="text-center fw-light">รหัสประจำคิว</th>
                            <th class="text-center fw-light">Queue reserve</th>
                            <th class="text-center fw-light">Action</th>
                        </tr>
                    </thead>
