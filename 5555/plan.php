...
// SQL ดึงข้อมูลแผนเดินรถ พร้อมชื่อเส้นทางจาก location
$sql = "
SELECT 
    bp.bp_id AS id,
...
// ดึงข้อมูลพนักงานสำรอง/ไม่พร้อม
...
if (mysqli_num_rows($result) === 0) {
    echo "<tr><td colspan='9' class='text-center text-danger'>ไม่มีข้อมูลแผนเดินรถ</td></tr>";
} else {
    while ($row = mysqli_fetch_assoc($result)) {
        echo "<tr>
                ...
                <td>{$row['br_id']}</td>
                <td>{$row['licen']}</td>
                <td>" . htmlspecialchars($row['emM']) . " <span class='badge bg-primary'>{$row['emM_que']}</span></td>
                <td>" . htmlspecialchars($row['emX1']) . " <span class='badge bg-secondary'>{$row['emX1_que']}</span></td>
                <td>" . htmlspecialchars($row['emX2']) . " <span class='badge bg-secondary'>{$row['emX2_que']}</span></td>
                <td>" . htmlspecialchars($row['emC']) . " <span class='badge bg-warning text-dark'>{$row['emC_que']}</span></td>
            </tr>";
        $i++;
    }
}
...
if (mysqli_num_rows($result_main) > 0) {
    while ($row_main = mysqli_fetch_assoc($result_main)) {
        echo "<div class='col-md-3 mb-2'><span class='fw-bold'>" . htmlspecialchars($row_main['em_name'] . ' ' . $row_main['em_surname']) . "</span> <span class='badge bg-primary'>" . htmlspecialchars($row_main['em_queue']) . "</span></div>";
    }
} else {
    echo "<div class='col-12 text-muted'>ไม่มีข้อมูล</div>";
}
?>
    </div>
...
if (mysqli_num_rows($result_ex) > 0) {
    while ($row_ex = mysqli_fetch_assoc($result_ex)) {
        echo "<div class='col-md-3 mb-2'><span class='fw-bold'>" . htmlspecialchars($row_ex['em_name'] . ' ' . $row_ex['em_surname']) . "</span> <span class='badge bg-secondary'>" . htmlspecialchars($row_ex['em_queue']) . "</span></div>";
    }
} else {
    echo "<div class='col-12 text-muted'>ไม่มีข้อมูล</div>";
}
...
if (mysqli_num_rows($result_coach) > 0) {
    while ($row_coach = mysqli_fetch_assoc($result_coach)) {
        echo "<div class='col-md-3 mb-2'><span class='fw-bold'>" . htmlspecialchars($row_coach['em_name'] . ' ' . $row_coach['em_surname']) . "</span> <span class='badge bg-warning text-dark'>" . htmlspecialchars($row_coach['em_queue']) . "</span></div>";
    }
} else {
    echo "<div class='col-12 text-muted'>ไม่มีข้อมูล</div>";
}
