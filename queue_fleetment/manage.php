...
$route = [2, 3, 4];  // รหัสสายรถที่ใช้
$normal_code = [3, 2, 1];

// ดึงชื่อสายรถจริง ๆ จากฐานข้อมูล
$routeNames = [];
if (!empty($route)) {
    $route_in = implode(',', array_map('intval', $route));
    $sql = "SELECT br_id, CONCAT(loc_start.locat_name_th, ' - ', loc_end.locat_name_th) AS route_name
            FROM bus_route br
            JOIN location loc_start ON br.loc_start_id = loc_start.locat_id
            JOIN location loc_end ON br.loc_end_id = loc_end.locat_id
            WHERE br.br_id IN ($route_in)";
...
// ส่วนเดิมของคุณเรียกฟังก์ชันและจัดการข้อมูลแผนเดินรถ
